...
async function squery(queryString){
...
// 	})
//     .then((data) => {
//         curExtract = data[0].Extract;
//         curBookID = data[0].BookID;
//         return Squery(`SELECT BookName FROM books WHERE BookID = ${curBookID};`);
//     })
//     .then((data) => {
...
//         }
//         wrongBookIDs = tempArr.slice(0, 4);
...
//     .then((data) => {
//         options.push(data[0].BookName);
//         options.push(curBookName);
//         for(let i=options.length-1; i>0; i--){
//             const j=Math.floor(Math.random()*(i + 1));
//             [options[i], options[j]] = [options[j], options[i]];
//         }
//         res.render('main.ejs', {options, curBookName, curExtract});
//     })
// 	.catch((err) => {
// 		console.log(err);
// 	});
...
async function mergedProcess(filepath, authorName, authorAbout, bookName, bookAbout, bookPubYear) {
    try {
        ...
        const shuffledExtracts = await shuffle(selectedExtracts, 1000);
        ...
        if (haveBook.length == 0) {
            ...
            for (let i = 0; i < shuffledExtracts.length; i++) {
                await squery(`INSERT INTO extracts (extractText, extractBookID, extractRating) VALUES ("${shuffledExtracts[i]}", ${bookID}, 2000);`);
            }
        }
    } catch (error) {
